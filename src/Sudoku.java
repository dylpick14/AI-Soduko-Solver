...
// File: Sudoku.java
//
// Group Member Names: Dylan Attridge & Cole Gunter
// Due Date:11/24/2019
// 
//
...
public class Sudoku
{
	...
	private static boolean isSolved(SudokuCoord[][] grid)
	{
		...
		//forloops to check and see if all numbers 1-9 are used correctly if they are return true if they are not return false
	...
	//Method that returns a list of possible values in a cell which is used to help solve all search methods
	...
	static boolean solveSudokuForwardChecking(SudokuCoord[][] grid) {
		...
		if (variable == null)
			return true; // Success!
		else if (variable.isAssigned()) { //Checks to see if a variable is assigned
			System.err.println("The selected variable has already been assigned: " + variable);
			System.exit(0);
		}
		
		// goes through all the values possible in the list
	...
	/////////////////////////////////////////////////////////////////////
	//searches for any unassigned entries. If there are there will be a location that is set
	//and returns the row and column. If there isnt any unassigned entries, null is returned
	/////////////////////////////////////////////////////////////////////
	...
	static SudokuCoord MyOriginalStaticOrderingOpt2(SudokuCoord grid[][])
	{
		...
		//checks through the grid to see if there are any unassigned variables,
		//if there is a row and column will be returned if not it will return null
		int i = 0;
		while(i < grid.length){
		    int j = 0;
	...
	static SudokuCoord MyOriginalRandomOrderingOpt3(SudokuCoord grid[][])
	{
		...
		//this one may take a while with it being random variables
		...
		int i = grid.length * grid.length * grid.length;
		while(i >=0 ) {
        	...
        	if (grid[randomNumber1][randomNumber2].isAssigned()) {
	...
	static SudokuCoord MyMinRemainingValueOrderingOpt4(SudokuCoord grid[][])
	{
		...
		//cycles through the grid to find the minimum possibilities from a cell and choose that one
		for (int row = 0; row < grid.length; row++) {
			for (int col = 0; col < grid.length; col++) {
				if (grid[row][col].value == SudokuCoord.UNASSIGNED) {
					tmp = GetPossibleValues(grid).size();
					if(tmp<min) {
						min=tmp;
						return grid[row][col];
					}
